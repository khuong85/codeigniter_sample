...
class Template {
	...
	{	
        ...
        $this->set('base_url', $this->ci->config->item('base_url'));
        $this->set('username', ($this->ci->session->userdata('username')?$this->ci->session->userdata('username'):$this->ci->session->userdata('email')));
        $this->set('layout', $this->ci->session->userdata('group'));
	...
	public function message() 
	{
		...
		if (!empty($message))
		{
			...
			$type = $temp_message[0];
			$message =(isset( $temp_message[1])?$temp_message[0]:'');
